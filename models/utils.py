...
def load_data(batch_size=1, suffix='', folder_name=''):
    loc_train = np.load('data/' + folder_name + '/loc_train' + suffix + '.npy')
    vel_train = np.load('data/' + folder_name + '/vel_train' + suffix + '.npy')
    edges_train = np.load('data/' + folder_name + '/edges_train' + suffix + '.npy')

    loc_valid = np.load('data/' + folder_name + '/loc_valid' + suffix + '.npy')
    vel_valid = np.load('data/' + folder_name + '/vel_valid' + suffix + '.npy')
    edges_valid = np.load('data/' + folder_name + '/edges_valid' + suffix + '.npy')

    loc_test = np.load('data/' + folder_name + '/loc_test' + suffix + '.npy')
    vel_test = np.load('data/' + folder_name + '/vel_test' + suffix + '.npy')
    edges_test = np.load('data/' + folder_name + '/edges_test' + suffix + '.npy')
...
def nll_gaussian(preds, target, variance, add_const=False):
    ...
    if add_const:
        ...
        neg_log_p += const
    print("target size : "+str(target.size(0) * target.size(1)))
    print(neg_log_p.size())
    return neg_log_p.sum() / (target.size(0) * target.size(1)) # sum/(16*12)


def edge_accuracy(preds, target):
    _, preds = preds.max(-1) 
    correct = preds.float().data.eq( #compares target and pred matrices
