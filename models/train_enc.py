...
parser.add_argument('--folder-name', type=str, default='with4actions/paperdata/testset0',
...
def train(epoch, best_val_accuracy):
    ...
    for batch_idx, (data, target) in enumerate(train_loader): 
        ...
        data, target = Variable(data), Variable(target) # target is [128x132]. 
        optimizer.zero_grad()
        # Limit to same length as train sequence
        data = data[:, :, :args.timesteps, :].contiguous()
        output = model(data, rel_rec, rel_send) #output is [128x132x2]. 
        
        # Flatten batch dim
        output = output.view(-1, args.num_classes) # output becomes [16896,2] (flattened by 128x132)
        target = target.view(-1) #target becomes [16896,1] (flattened by 128x132)

        loss = F.cross_entropy(output, target)
        loss.backward()
        optimizer.step()

        pred = output.data.max(1, keepdim=True)[1] #pred is [16896,1]
        correct = pred.eq(target.data.view_as(pred)).cpu().sum() 
        acc = correct / pred.size(0) # accuracy is number of hits over the total number of guesses
    

        loss_train.append(loss.data[0])
        acc_train.append(acc)

        loss_train_writer = loss 
    ...
    writer_valid.add_scalars(f'loss/train', {
            'training': loss_train_writer,
            'validation': loss,
    }, epoch)
    writer_valid.flush()
...
def test():
    ...
    for batch_idx, (data, target) in enumerate(test_loader):
        ...
        for i in range(len(pred[:])):
            if pred[i,0]==1 and target.data[i]==1:
                true_positives+=1
            elif pred[i,0]==1 and target.data[i]==0:
                false_positives+=1
            elif pred[i,0]==0 and target.data[i]==1:
                false_negatives+=1
            f1score=2/((true_positives+false_positives)/true_positives+(false_negatives+true_positives)/true_positives)
        f1score_test.append(f1score)
        
    writer_test.add_scalar("Loss/test", loss, epoch)
    writer_test.flush()
    ...
    print('loss_test: {:.10f}'.format(np.mean(loss_test)),
          ...
          'f1score_test: {:.10f}'.format(np.mean(f1score_test)))
    if args.save_folder:
        print('--------------------------------', file=log)
        ...
        print('loss_test: {:.10f}'.format(np.mean(loss_test)),
              'acc_test: {:.10f}'.format(np.mean(acc_test)), 
              'f1score_test: {:.10f}'.format(np.mean(f1score_test)), file=log)
        log.flush()
