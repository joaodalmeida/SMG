...
class MLPDecoder(nn.Module):
    ...
    def __init__(self, n_in_node, edge_types, msg_hid, msg_out, n_hid,
        ...
        self.out_fc3 = nn.Linear(n_hid, 3) 
    ...
    def single_step_forward(self, single_timestep_inputs, rel_rec, rel_send,
        ...
        pred = self.out_fc3(pred) # [32,5,12,3]
        return single_timestep_inputs[:,:,:,:3] + pred 

    def forward(self, inputs, rel_type, rel_rec, rel_send, pred_steps=1):
        # NOTE: Assumes that we have the same graph across all samples.
        #SHAPES: inputs: [32,12,50,4] ; rel_type: [32,132,2] ; rel_rec: [132,12] ; rel_send: [132,12]; pred_steps:int
        inputs = inputs.transpose(1, 2).contiguous() # transpose inputs from [32,12,50,4] to [32,50,12,4]
        
        sizes = [rel_type.size(0), inputs.size(1), rel_type.size(1),
                 rel_type.size(2)] #{32,12,49,4}
        rel_type = rel_type.unsqueeze(1).expand(sizes) # rel_type goes from [32,132,2] to [32,50,132,2]. Unsqueeze(1) creates a new dimension at position 1
        ...
        last_pred = inputs[:, 0::pred_steps, :, :] # [32,5,12,4] 0::12 means starting from 0, 1 every 12 points, so inputs[:,{0,12,24,36,48},:,:]
        #pred_and_actions = torch.cat([last_pred,inputs[:,0::pred_steps,:,3:]],dim=-1)
        curr_rel_type = rel_type[:, 0::pred_steps, :, :] # [32,5,132,2] 0::10 means starting from 0, 1 every 10 points, so inputs[:,{0,10,20,30,40},:,:]
        # NOTE: Assumes rel_type is constant (i.e. same across all time steps).
        # Run n prediction steps
        for step in range(0, pred_steps): 
            #last_pred = self.single_step_forward(pred_and_actions, rel_rec, rel_send,
            #                                     curr_rel_type) # [32,5,12,3]
            last_pred = self.single_step_forward(last_pred, rel_rec, rel_send,
                                                 curr_rel_type) # [32,5,12,3] # calculates the next 5 points (10 times in the for loop)
            # so first run calculates {1,11,21,31,41}, 2nd run is {2,12,22,32,42}, 10th and last run is {10,20,30,40,50}

            last_pred = torch.cat([last_pred,inputs[:,step::pred_steps,:,3:]],dim=-1) # changed by me, line was not here before # [32,5,12,7]
            preds.append(last_pred) # in the end preds is list with { [32,{1,11,21,31,41},12,7] ; [32,{2,12,22,32,42},12,7] ; ... ; [32,{10,20,30,40,50},12,7]]  }

        sizes = [preds[0].size(0), preds[0].size(1) * pred_steps,
                 preds[0].size(2), preds[0].size(3)] # [32,50,12,7] = [32,5*10,12,7]
        ...
        for i in range(len(preds)): #len(preds)=pred_step = 10
            output[:, i::pred_steps, :, :] = preds[i] 

        pred_all = output[:, :(inputs.size(1) - 1), :, :] # [32,49,12,7]
        return pred_all.transpose(1, 2).contiguous() # [32,12,49,7]
...
class RNNDecoder(nn.Module):
    ...
    def single_step_forward(self, inputs, rel_rec, rel_send,
        ...
        pred = inputs[:,:,:3] + pred # changed by me from pred = inputs + pred

        return pred, hidden # pred is [32,12,3] ([32,12,4] before I changed it) and hidden is [32,12,256]
    ...
    def forward(self, data, rel_type, rel_rec, rel_send, pred_steps=1,
        ...
        inputs = data.transpose(1, 2).contiguous() # [32,50,12,4]
        time_steps = inputs.size(1)
        ...
        for step in range(0, inputs.size(1) - 1): # for step in range(0, 49):
            # Training: burn_in is True in training, burn_in_steps is 50-10=40
            # Validation and Test: burn_in is False, step % pred_steps = 0
            if burn_in: #burn_in allows the model to look at the first (50-prediction_step) and then predicts the following prediction_steps
                if step <= burn_in_steps:
                    ins = inputs[:, step, :, :] 
                else:
                    ins = pred_all[step - 1]
            else: # if there is no burn in, model behaves in the same way as mlp: gives the model timesteps {0,10,20,30,40} and predicts the others from them.
                assert (pred_steps <= time_steps)
            ...
            pred, hidden = self.single_step_forward(ins, rel_rec, rel_send,
                                                    rel_type, hidden)
            pred = torch.cat([pred,inputs[:,step,:,3:]],dim=-1) # by me to add actions. pred goes from [32,12,3] to [32,12,4]
